...
int64_t vorbis_duration(const char * url) {
	av_register_all();
...
int vorbis_check(const char * url) {
	av_register_all();
	avcodec_register_all();
	AVFormatContext *ctx = NULL;
	int err = avformat_open_input(&ctx,url,NULL,NULL);
	...
	if (strcmp(codec->name , "vorbis")==0) {
		//Either image data or we are some kind of multimedia codec with vorbis audio
		if (ctx->nb_streams > 1) {
...
//Returns whether or not the file is Vorbis based on the streams that reside in it
func IsVorbis(filename string) bool {
	cs := C.CString(filename)
	defer C.free(unsafe.Pointer(cs))
	if r := C.vorbis_check(cs); r >= 0 {
		return true
	}
	return false
}
...
func VorbisDuration(filename string) time.Duration {
	cs := C.CString(filename)
	defer C.free(unsafe.Pointer(cs))
	return time.Duration(C.vorbis_duration(cs)) * 1000
}

//Extracts the first image we find in the Ogg file
